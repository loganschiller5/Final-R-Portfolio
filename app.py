...
# ─────────────────────────────────────────────
# WATCHLIST (5 stocks on home page)
# ─────────────────────────────────────────────
WATCHLIST = [
...
PORTFOLIO_TICKERS = ["AAPL", "MSFT", "GOOGL", "NVDA", "JPM"]
PORTFOLIO_WEIGHTS = {"AAPL": 0.25, "MSFT": 0.25, "GOOGL": 0.20, "NVDA": 0.20, "JPM": 0.10}
...
[data-testid="metric-container"]:hover {
    ...
    box-shadow: 0 0 18px rgba(59,130,246,0.10) !important;
}
...
/* Stock card */
.stock-card-wrap { position: relative; height: 100%; }
.stock-card {
    background: linear-gradient(145deg, #0f1623 0%, #0b101c 100%);
    border: 1px solid #1a2540;
    border-radius: 18px;
...
.sc-name  { font-size: 0.82rem; color: #475569; margin-bottom: 0.15rem; font-weight: 500; }
.sc-tick  { font-size: 1.7rem; font-weight: 800; color: #f1f5f9; letter-spacing: -0.03em; line-height: 1.1; }
.sc-price { font-size: 1.1rem; font-weight: 700; color: #f1f5f9; margin-top: 0.6rem; }
.sc-pos   { color: #10b981; font-size: 0.82rem; font-weight: 600; }
.sc-neg   { color: #ef4444; font-size: 0.82rem; font-weight: 600; }
...
.badge-buy  { background: rgba(16,185,129,0.12); color: #10b981; border: 1px solid rgba(16,185,129,0.3); }
.badge-sell { background: rgba(239,68,68,0.12);  color: #ef4444; border: 1px solid rgba(239,68,68,0.3); }
.badge-hold { background: rgba(245,158,11,0.12); color: #f59e0b; border: 1px solid rgba(245,158,11,0.3); }
...
.ic-label { font-size: 0.65rem; text-transform: uppercase; letter-spacing: 0.1em; color: #1e3a5f; font-weight: 700; margin-bottom: 0.4rem; }
.ic-val   { font-size: 1.05rem; font-weight: 800; line-height: 1.1; }
.ic-sub   { font-size: 0.78rem; color: #334155; margin-top: 0.5rem; line-height: 1.7; }
...
.rec-panel-buy  { border: 1px solid rgba(16,185,129,0.22); }
.rec-panel-sell { border: 1px solid rgba(239,68,68,0.22); }
.rec-panel-hold { border: 1px solid rgba(245,158,11,0.22); }
.rec-panel::after {
    content: '';
    position: absolute;
    top: 0; left: 0;
...
BASE = dict(
    ...
    legend=dict(bgcolor="rgba(15,22,35,0.9)", bordercolor="#1a2540",
...
def get_prices(ticker: str, period: str = "6mo") -> pd.DataFrame:
    try:
...
def calc_ann_vol(s: pd.Series) -> float:
...
def vol_sig(df):
    v = calc_ann_vol(df["Close"].squeeze())
...
def port_calc(tickers: tuple, weights: tuple, period: str):
    ...
    w_arr  = np.array([wd.get(t, 0) for t in prices.columns], dtype=float)
    w_arr /= w_arr.sum()
    pr     = rets.dot(w_arr)
    ...
    return dict(total=total, av=av, sharpe=sharpe, bt=bt,
                outperf=total - bt, cum=cum, bc=bc,
                rets=rets, pr=pr, prices=prices, dd=dd)
...
def make_sparkline(spark: pd.Series, chg: float, idx: int) -> str:
    if spark is None or len(spark) < 2:
        return ""
    norm  = (spark - spark.min()) / (spark.max() - spark.min() + 1e-9)
    pts   = list(norm.values)
    W, H  = 180, 44
    step  = W / max(len(pts) - 1, 1)
    path  = " ".join(
        f"{'M' if j == 0 else 'L'}{x*step:.1f},{(1-y)*(H-4)+2:.1f}"
        for j, (x, y) in enumerate(zip(range(len(pts)), pts))
    )
    lx = (len(pts)-1) * step
    ly = (1 - pts[-1]) * (H-4) + 2
    fill = f"{path} L{lx:.1f},{H} L0,{H} Z"
    sc   = "#10b981" if chg >= 0 else "#ef4444"
    gid  = f"spg{idx}"
    return f"""
    <svg viewBox="0 0 {W} {H}" width="100%" height="{H}"
         preserveAspectRatio="none" style="display:block;margin-top:8px">
      <defs>
        <linearGradient id="{gid}" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stop-color="{sc}" stop-opacity="0.22"/>
          <stop offset="100%" stop-color="{sc}" stop-opacity="0.01"/>
        </linearGradient>
      </defs>
      <path d="{fill}" fill="url(#{gid})"/>
      <path d="{path}" fill="none" stroke="{sc}"
            stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
      <circle cx="{lx:.1f}" cy="{ly:.1f}" r="2.5" fill="{sc}"/>
    </svg>"""
...
def render_home():
    ...
    with col_mkt:
        ...
        for sym, label in indices.items():
            ...
            if px_ is not None:
                ...
                arrow = "▲" if chg_ >= 0 else "▼"
                parts.append(
                    f"<div class='mkt-item'>"
                    f"<span class='mkt-label'>{label}</span>"
                    f"<span class='mkt-val'>${px_:,.2f}</span>"
                    f"<span class='{css}'>{arrow} {abs(chg_)*100:.2f}%</span>"
                    f"</div>"
                )
        st.markdown(
            f"<div class='mkt-bar' style='margin-top:.3rem'>{''.join(parts)}</div>",
            unsafe_allow_html=True,
        )

    # ── Section label ──
    st.markdown(
        "<div class='section-label'>Watchlist — click to explore</div>",
        unsafe_allow_html=True,
    )
    ...
    for i, stock in enumerate(WATCHLIST):
        t = stock["ticker"]
        px_, chg_, spark = get_quick(t)

        chg_css = "sc-pos" if (chg_ or 0) >= 0 else "sc-neg"
        arrow   = "▲" if (chg_ or 0) >= 0 else "▼"
        px_str  = f"${px_:,.2f}" if px_ else "—"
        chg_str = f"{arrow} {abs(chg_ or 0)*100:.2f}%" if chg_ is not None else "—"
        spark_svg = make_sparkline(spark, chg_ or 0, i)

        with cols[i]:
            # Render the visual card
            st.markdown(f"""
            <div class="stock-card-wrap">
              <div class="stock-card" id="card_{t}">
                <div class="sc-sector">{stock['sector']}</div>
                <div class="sc-name">{stock['name']}</div>
                <div class="sc-tick">{t}</div>
                <div class="sc-price">{px_str}</div>
                <div class="{chg_css}">{chg_str}
                    <span style="color:#1e3a5f;font-size:0.68rem;margin-left:4px">1M</span>
                </div>
                {spark_svg}
              </div>
            </div>
            ...
            # Invisible overlay button that triggers navigation
            if st.button(f"View {t}", key=f"btn_{t}", use_container_width=True):
                st.session_state.selected_stock = t
                st.rerun()

    # ── Portfolio Summary ──
    st.markdown(
        "<div class='section-label' style='margin-top:2rem'>Portfolio Snapshot · 1Y</div>",
        unsafe_allow_html=True,
    )

    m = port_calc(
        tuple(PORTFOLIO_TICKERS),
        tuple(PORTFOLIO_WEIGHTS[t] for t in PORTFOLIO_TICKERS),
        "1y",
    )
...
def render_stock(ticker: str):
    ...
    m_macd, m_sig, m_hist = calc_macd(close)
    b_up, b_mid, b_lo     = calc_bb(close)

    px_now = tr["px"]
    px_ago = float(close.iloc[0])
    chg_6m = (px_now - px_ago) / px_ago
    ...
    l52     = info.get("fiftyTwoWeekLow",    None)
    beta    = info.get("beta",               None)
    dy      = info.get("dividendYield",      None)

    # ── Header ──
    ch_col   = "#10b981" if chg_6m >= 0 else "#ef4444"
    ch_arrow = "▲" if chg_6m >= 0 else "▼"

    hc1, hc2, hc3 = st.columns([3, 1, 1])
    with hc1:
        st.markdown(f"""
        <div>
            <div style="color:#1e3a5f;font-size:0.68rem;text-transform:uppercase;
                        letter-spacing:.1em;font-weight:700;margin-bottom:4px">{sector}</div>
            <div style="font-size:2rem;font-weight:800;color:#f1f5f9;
                        letter-spacing:-0.04em;line-height:1.1">{name}</div>
            <div style="margin-top:3px">
                <span style="color:#3b82f6;font-weight:700;font-size:0.9rem">{ticker}</span>
                <span style="color:#1a2540;margin:0 6px">·</span>
                <span style="color:#334155;font-size:0.85rem">{bignum(mktcap)} Market Cap</span>
            </div>
        </div>""", unsafe_allow_html=True)
    with hc2:
        st.markdown(f"""
        <div style="text-align:right;padding-top:.4rem">
            ...
            <div style="color:{ch_col};font-size:0.9rem;font-weight:700">
                {ch_arrow} {abs(chg_6m)*100:.2f}%
                <span style="color:#1e3a5f;font-size:0.7rem"> 6M</span>
            </div>
        </div>""", unsafe_allow_html=True)
    with hc3:
        st.markdown(
            f"<div style='text-align:right;padding-top:.7rem'>"
            f"<span class='badge {rec_badge}' style='font-size:0.9rem;padding:8px 22px'>⬟ {rec}</span>"
            f"</div>",
            unsafe_allow_html=True,
        )
    ...
    # ──────────────────────────────────────────
    with tab_a:
        # Chart type — Candlestick / Line only
        ct_col, _ = st.columns([2, 6])
        with ct_col:
        ...
        if chart_choice == "Candlestick":
            fig.add_trace(go.Candlestick(
                x=df6.index,
                open=df6["Open"].squeeze(), high=df6["High"].squeeze(),
                low=df6["Low"].squeeze(),   close=close,
            ...
            fig.add_trace(go.Scatter(
                x=df6.index, y=close, name=ticker,
                line=dict(color="#3b82f6", width=2.3), showlegend=False,
            ), row=1, col=1)

        # Moving averages
        fig.add_trace(go.Scatter(
            x=df6.index, y=close.rolling(20).mean(), name="MA 20",
            line=dict(color="#f59e0b", width=1.5, dash="dot"),
        ), row=1, col=1)
        fig.add_trace(go.Scatter(
            x=df6.index, y=close.rolling(50).mean(), name="MA 50",
            line=dict(color="#ef4444", width=1.5, dash="dot"),
        ), row=1, col=1)

        # Bollinger Bands
        fig.add_trace(go.Scatter(
            x=df6.index, y=b_up, name="BB Upper",
            line=dict(color="#8b5cf6", width=1, dash="dash"),
        ), row=1, col=1)
        fig.add_trace(go.Scatter(
            x=df6.index, y=b_lo, name="BB Lower",
            line=dict(color="#8b5cf6", width=1, dash="dash"),
        ...
        vc = ["#10b981" if float(c_) >= float(o_) else "#ef4444"
              for c_, o_ in zip(df6["Close"].squeeze(), df6["Open"].squeeze())]
        fig.add_trace(go.Bar(
            x=df6.index, y=df6["Volume"].squeeze(),
        ...
        with ic1:
            ...
            fr = go.Figure()
            fr.add_hrect(y0=70, y1=100, fillcolor="rgba(239,68,68,0.05)", line_width=0)
            fr.add_hrect(y0=0,  y1=30,  fillcolor="rgba(16,185,129,0.05)", line_width=0)
        ...
        with ic2:
            ...
            fm.add_trace(go.Bar(x=df6.index, y=m_hist, marker_color=hc, opacity=0.7, name="Histogram"))
            fm.add_trace(go.Scatter(x=df6.index, y=m_macd,
                                    line=dict(color="#3b82f6", width=2), name="MACD"))
            fm.add_trace(go.Scatter(x=df6.index, y=m_sig,
        ...
        fg = go.Figure(go.Indicator(
            ...
            gauge=dict(
                ...
                steps=[
                    dict(range=[0,  25], color="#051209"),
                    dict(range=[25, 40], color="#120d02"),
                    dict(range=[40, 80], color="#120202"),
                ],
            ),
            title=dict(
                text=f"Level: <b style='color:{vl['col']}'>{vl['lbl']}</b>",
        ...
        with sc1:
            ...
            <div class="info-card">
                ...
                <div class="ic-sub">
                    ...
                    MA 50 &nbsp;<b style="color:#ef4444">${tr['m50']:,.2f}</b>
                </div>
            </div>""", unsafe_allow_html=True)
        with sc2:
            st.markdown(f"""
            <div class="info-card">
                <div class="ic-label">⚡ Momentum (RSI)</div>
                <div class="ic-val" style="color:{mo['col']}">{mo['lbl']}</div>
                <div class="ic-sub">
                    RSI &nbsp;<b style="color:#f1f5f9">{mo['val']:.1f}</b><br>
        ...
        with sc3:
            st.markdown(f"""
            <div class="info-card">
                <div class="ic-label">🌊 Volatility</div>
        ...
        <div class="rec-panel {rec_panel}">
            <div style="display:flex;align-items:center;gap:1.1rem;margin-bottom:.75rem">
                ...
                <div>
                    <div style="color:#f1f5f9;font-weight:700;font-size:0.95rem;margin-bottom:3px">
                        Trading Recommendation
                    </div>
                    <div style="color:#475569;font-size:0.84rem">{rec_reason}</div>
                </div>
            </div>
            <p style="color:#1e3a5f;font-size:0.68rem;margin:0;
                      border-top:1px solid #111827;padding-top:.65rem">
                ⚠ Based on technical indicators only. Not investment advice.
                Conduct your own due diligence before investing.
            </p>
        </div>""", unsafe_allow_html=True)

        # Fundamentals
        if any([pe, eps, h52, l52, beta, dy]):
            st.markdown("---")
            st.markdown('<p class="sec-title">Fundamentals</p>', unsafe_allow_html=True)
            fb1, fb2, fb3, fb4, fb5, fb6 = st.columns(6)
            fb1.metric("P/E Ratio",  f"{pe:.1f}×"     if pe   else "—")
            fb2.metric("EPS (TTM)",  f"${eps:.2f}"     if eps  else "—")
    ...
    with tab_p:
        ...
        if m is None:
            ...
            # Weight pills
            pills = " ".join(
                f"<span class='weight-pill'>{t} {PORTFOLIO_WEIGHTS[t]*100:.0f}%</span>"
                for t in PORTFOLIO_TICKERS
            )
            st.markdown(
                f"<div style='margin:.2rem 0 1rem;display:flex;flex-wrap:wrap;gap:5px'>{pills}</div>",
                unsafe_allow_html=True,
            )
            ...
            pk2.metric("vs Benchmark",    pct(m["outperf"]),
            ...
            with pt1:
                ...
                if bc_ is not None:
                    fpp.add_trace(go.Scatter(
                        x=bc_.index, y=(bc_-1)*100, name="SPY",
                ...
                frsh = go.Figure()
                frsh.add_trace(go.Scatter(
                    x=rsh.index, y=rsh, name="Rolling Sharpe (21d)",
            ...
            with pt2:
                ...
                with ac1:
                    fpie = go.Figure(go.Pie(
                        ...
                        values=[PORTFOLIO_WEIGHTS[t]*100 for t in PORTFOLIO_TICKERS],
                        hole=0.58,
                        marker=dict(
                            colors=COLORS[:len(PORTFOLIO_TICKERS)],
                    ...
                    fpie.update_layout(
                        ...
                        title=dict(text="Allocation Weights",
                ...
                with ac2:
                    ind_r = {}
                    for t_ in PORTFOLIO_TICKERS:
                        dft = get_prices(t_, pper)
                        if not dft.empty:
                    ...
                    fbar = go.Figure(go.Bar(
                        ...
                        marker_color=[COLORS[0] if v >= 0 else "#ef4444" for v in s_r.values()],
                ...
                for t_ in PORTFOLIO_TICKERS:
                    ...
                    rv   = (dft["Close"].squeeze().iloc[-1] /
                            dft["Close"].squeeze().iloc[0] - 1) * 100
                    rc_  = "#10b981" if rv >= 0 else "#ef4444"
                    wt_  = f"{PORTFOLIO_WEIGHTS.get(t_, 0)*100:.0f}%"
                    st.markdown(f"""
                    <div class="holding-row">
                        <div style="display:flex;align-items:center;gap:.9rem">
                            <span style="color:#f1f5f9;font-weight:700">{t_}</span>
                            <span class="weight-pill">{wt_}</span>
                            <span style="color:#1e3a5f;font-size:0.76rem">
            ...
            with pt3:
                ...
                with rr1:
                    ...
                    fh.add_trace(go.Histogram(
                        x=pr_*100, nbinsx=40, name="Daily Returns",
                        marker_color="#3b82f6", opacity=0.7,
                    ))
                    fh.add_vline(x=float(pr_.mean()*100),
                                 line_dash="dot", line_color="#10b981", line_width=1.5,
                ...
                pi   = "✅" if out_ > 0    else "❌"
                ri   = "✅" if av_ < 0.20  else "⚠️"
                si   = "✅" if sh_ >= 1    else ("⚠️" if sh_ >= .5 else "❌")
                ol   = "Outperformed" if out_ > 0 else "Underperformed"
                sl   = ("Excellent" if sh_ >= 2 else "Good" if sh_ >= 1
                        else "Fair" if sh_ >= .5 else "Poor — consider rebalancing")
                st.markdown(f"""
                <div class="interp-box">
                    <p style="color:#1e3a5f;font-size:0.68rem;text-transform:uppercase;
                              letter-spacing:.1em;font-weight:700;margin:0 0 .8rem">
...
if st.session_state.selected_stock:
    render_stock(st.session_state.selected_stock)
else:
    render_home()
